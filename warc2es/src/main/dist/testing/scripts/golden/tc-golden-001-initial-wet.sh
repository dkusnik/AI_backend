#!/bin/bash
# tc-golden-001-initial-wet.sh — Golden cycle with clean-artifact guard
# Uses default MULTI_WARC output and directory-aware warc_count.
# Cleans stale artifacts at start to prevent format conflicts across runs.
# See tc-golden-002 for the directory-output variant with explicit structure assertions.
...
GOLDEN_INDEX="${GOLDEN_INDEX:-nac-data-golden-test-$RUN_ID}"
GOLDEN_DIR="$PROJECT_ROOT/target/testing/tmp/golden"
mkdir -p "$GOLDEN_DIR"
# Clean stale artifacts from previous runs to prevent format conflicts
# (e.g. plock1.wet.gz might be a directory from MULTI_WARC or a file from WARC).
rm -rf "$GOLDEN_DIR"/plock*.wet.gz "$GOLDEN_DIR"/plock*.doet.gz \
       "$GOLDEN_DIR"/plock-merged.doet.gz "$GOLDEN_DIR"/plock-diff.doet.gz \
       "$GOLDEN_DIR"/.run-id
...
trap 'curl -sS -o /dev/null -X DELETE "${ES_URL}/_data_stream/${GOLDEN_INDEX}" 2>/dev/null || true' EXIT INT TERM
...
if [[ "$health_rc" -eq 2 ]]; then
    echo "TESTCASE|golden-es-unavailable|SKIP|reason=no-es"
    exit 0
fi
...
stage_1_initial_cycle() {
    ...
    assert_exact_count "plock1.wet.gz" "$wet_count" "$EXPECTED_WET1" || return 1
...
stage_2_incremental_cycle() {
    ...
    assert_exact_count "plock2.wet.gz" "$wet_count" "$EXPECTED_WET2" || return 1
...
stage_recalculate_counts() {
    ...
    assert_exact_count "recalc plock1.wet.gz" "$c_wet1" "$EXPECTED_WET1" || return 1
    assert_exact_count "recalc plock1.doet.gz" "$c_doet1" "$EXPECTED_DOET1" || return 1
    assert_exact_count "recalc plock2.wet.gz" "$c_wet2" "$EXPECTED_WET2" || return 1
